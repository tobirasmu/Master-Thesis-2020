...
For the second method that decomposes a pre-trained network, a 2D and 3D CNN is applied to each of the data set respectively. An illustration of the two networks are given in \autoref{fig:architecture_choices}.
\begin{figure}
    \centering
    \captionsetup{width=.95\linewidth}
    \includegraphics[width=\linewidth]{Pics/05_methodology/network_choice_illustration.png}
    \caption{The architectures used for each of the data sets in the method that compresses a pre-trained network. The size of the intermediate matrices or tensors are given above them along with the number of channels. The red operations correspond to the operation named underneath them.}
