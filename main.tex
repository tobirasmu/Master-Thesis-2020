%-----------------------
% Document initialization
%-----------------------
\documentclass[pdftex, table, xcdraw]{article}
\usepackage[utf8]{inputenc}
\input{Tex/Preamble.tex}

%-----------------------
% Begin document
%-----------------------
\begin{document}

% Block commenting starting here !! (ends with \fi)
%\iffalse

\newgeometry{top = 2in}

% FRONTPAGE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{Tex/frontpage}
\clearpage

\restoregeometry

% Starting roman numbering
\setcounter{secnumdepth}{0}
\pagenumbering{Roman}

\sectionfont{\color{white}}


% ABSTRACT %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\afterpage{\blankpage}
\thispagestyle{abstract}
\input{Tex/0000_Abstract}
\clearpage

% PREFACE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\afterpage{\blankpage}
\thispagestyle{preface}
\input{Tex/000_Preface}
\clearpage

% Decreasing top margin
\newgeometry{top = 2in}

% CONTENTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\thispagestyle{contents}
\setcounter{tocdepth}{4}
\tableofcontents
\clearpage

% List of Tables %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{List of Figures}
\thispagestyle{lof}
\listoffigures
\clearpage

% List of figures %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{List of Tables}
\thispagestyle{lot}
\listoftables
\clearpage

%\fi %finish commenting

% ABREVIATIONS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\thispagestyle{abbreviations}
\input{Tex/00_Abreviations}
\clearpage

% Symbols %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\thispagestyle{symbols}
\input{Tex/0_Nomenclature}
\clearpage

% Going back to arabic numbering
\setcounter{secnumdepth}{4} %Means that 4 levels of sectioning comes into the table of contents
\pagenumbering{arabic}
\setcounter{page}{1}
% Setting section color and top margins
\newgeometry{top = 1.25in}
\sectionfont{\color{black}}
\pagestyle{main}

% Sections %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{Tex/1_Introduction}
\clearpage

\input{Tex/2_Theory}
\clearpage

\input{Tex/3_Previous_work}
\clearpage

\input{Tex/4_Data}
\clearpage

\input{Tex/5_Methods}
\clearpage

\input{Tex/6_Results}
\clearpage

\input{Tex/7_Discussion}
\clearpage

\input{Tex/8_Conclusion}
\clearpage


% References %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagestyle{emptyleft}
\printbibliography[heading = bibintoc, title={References}]
\clearpage

% Appendix %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\iffalse % Block commenting commencing here

\appendix
\pagenumbering{arabic}% resets `page` counter to 1
\renewcommand{\thepage}{A\arabic{page}}
\input{Tex/A_decomposition_estimation}
\clearpage

\pagenumbering{arabic}
\renewcommand{\thepage}{B\arabic{page}}
\input{Tex/B_tucker1_decomposition_convolution}
\clearpage

%\pagenumbering{arabic}
%\renewcommand{\thepage}{B\arabic{page}}
%\input{}
%\clearpage

%\fi
\end{document}
